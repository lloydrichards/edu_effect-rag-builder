...
describe("TokenChunker", () => {
  it.layer(makeWordTokenChunkerLive({ chunkSize: 5, chunkOverlap: 2 }))(
    (it) => {
      it.effect("chunks text into tokens with overlap", () =>
        Effect.gen(function* () {
  ...
  it.effect("fails on invalid config where chunk size is non-positive", () =>
    Effect.gen(function* () {
      const program = Effect.gen(function* () {
        const chunker = yield* Chunker;
        return yield* chunker.chunk("A B C");
...
describe("TokenChunker lesson alignment", () => {
  it.layer(
    ...
    it.effect(
      "matches the Lesson 01 worked example for character tokenization",
      () =>
        Effect.gen(function* () {
          const chunker = yield* Chunker;
